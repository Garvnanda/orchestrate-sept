...
- All money math, forecasting, plan ranking, validation and the `decision_explanation` text are deterministic Python — no model ever produces a number or an output field. Models only read messages and blank-amount images into a fixed JSON schema, which is normalized at the boundary; messages with facts not marked high confidence get a second read (a repeat `deepseek-v4-flash` call, or a different OpenRouter free model for messages AgentRouter blocks), and on disagreement the financially safer reading is kept.
